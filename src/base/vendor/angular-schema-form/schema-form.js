...
angular.module('schemaForm').provider('sfErrorMessage', function() {
  ...
  var defaultMessages = {
    'default': '表单字段未验证',
    0: '无效的类型, 类型应为 {{schema.type}}',
    1: '无法匹配枚举值: {{viewValue}}',
    10: '数据不匹配任何结构 "anyOf"',
    11: '数据不匹配某个结构 "oneOf"',
    12: '数据依赖超过某个结构从 "oneOf"',
    13: '数据匹配结构从 "not"',
    // Numeric errors
    100: '不是 {{schema.divisibleBy}} 中的值',
    101: '{{viewValue}} 小于 {{schema.minimum}} 允许范围',
    102: '{{viewValue}} 不可小于等于最小值 {{schema.minimum}}',
    103: '{{viewValue}} 超过了最大值 {{schema.maximum}}',
    104: '{{viewValue}} 不可超过了最大值 {{schema.maximum}}',
    105: '输入值不是数字',
    // String errors
    200: '字符太短 ({{viewValue.length}} 字符), 最少需要 {{schema.minLength}} 个',
    201: '字符太长 ({{viewValue.length}} 字符), 最多允许 {{schema.maxLength}} 个',
    202: '字符不匹配规则: {{schema.pattern}}',
    // Object errors
    300: '属性定义太少, 至少需要 {{schema.minProperties}} 个',
    301: '属性定义太多, 最多允许 {{schema.maxProperties}} 个',
    302: '必填',
    303: '额外的属性不允许',
    304: '依赖失败,键必须要由',
    // Array errors
    400: '数组太短 ({{value.length}}), 最小长度 {{schema.minItems}}',
    401: '数组太长 ({{value.length}}), 最大长度 {{schema.maxItems}}',
    402: '数组不唯一',
    403: '额外的数组不允许',
    // Format errors
    500: '格式验证失败',
